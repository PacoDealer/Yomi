...
**S112 re-ran S99's full-project-audit methodology, 13 sessions later, as an independent
re-verification.** Ran as an explicit multi-agent `Workflow` — 7 parallel research dimensions
(core/DB/app-entry/sync, extensions/plugins/browse/reader, library/more/history/onboarding,
docs-vs-code, App Store compliance, security/privacy, live simulator+build-health walkthrough), each
with its own adversarial verify pass re-checking every finding against the live file before it was
trusted. **Deliberately no fixes — findings only**, catalogued as new Known Issues rows 52-71 above.
...
hold). App Store compliance re-verified live and stayed clean. **20 new issues** (3 high / 8 medium /
9 low): headline items are a silently-swallowed `DatabaseManager.setup()` failure at launch (#52), a
live MangaDex-plugin bug that dead-ends on licensed/external chapters (#69, one of only 3 one-tap
sources), a small cluster of `clearLastRead`/`touchLastUpdated` call sites missing `markCloudDirty`
(#53-54, same bug class S105 fixed elsewhere), and a previously-undocumented privacy gap where the
Home Screen widget shows reading history regardless of App Lock/Secure Screen (#68). Rest is doc
staleness (#62-67) and small code-quality items (#55-61, #70-71). Full narrative in `Yomi/ROADMAP.md`'s
S112 entry.
...
| 52 | `DatabaseManager.shared.setup()` failure silently swallowed at app launch | `YomiApp.swift:150` calls `try? DatabaseManager.shared.setup()` inside `YomiApp.init()`. `setup()` (`DatabaseManager.swift:24-31`) is `throws` — opens the `DatabaseQueue` and runs all migrations — and `appDatabase` (`DatabaseManager.swift:9`) is a `nonisolated(unsafe)` implicitly-unwrapped optional, per the file's own comment "assigned once in `setup()` before any query runs." A real open/migration failure (corrupt DB file, disk full, a broken migration) leaves `appDatabase` `nil` with no error surfaced anywhere — every later query call site then force-unwraps a nil and the app crashes downstream instead of failing loudly at the real point of failure. No fallback, log, or UI exists around the `try?`. |
| 53 | `clearLastRead` never marks Manga/Novel dirty for CloudKit sync | `MangaQueries.clearLastRead` (`MangaQueries.swift:113`) and `NovelQueries.clearLastRead` (`NovelQueries.swift:98`) both run a raw UPDATE clearing `lastReadAt` with no `markCloudDirty` call, unlike their sibling `touchLastRead` functions (`MangaQueries.swift:103-110`, `NovelQueries.swift:88-95`) which do call it. `lastReadAt` is a genuinely synced field (`CloudSyncManager.swift:529,547`). Called from `HistoryView.swift:198/212` (manga) and `199/213` (novel) — clearing an item from History on one device silently never propagates the clear to other devices. |
| 54 | `touchLastUpdated` never marks Manga/Novel dirty for CloudKit sync | Same gap as #53, one severity notch down: `MangaQueries.touchLastUpdated` (`MangaQueries.swift:123`, called `UpdatesView.swift:192`) and `NovelQueries.touchLastUpdated` (`NovelQueries.swift:107`, called `UpdatesView.swift:275`) update `lastUpdatedAt` — also a synced field (`CloudSyncManager.swift:530,548`) — with no `markCloudDirty`. "New chapters found" state doesn't propagate across devices. |
...
| 56 | 4 more query functions are dead code, unreachable from any UI | `MangaQueries.insert(_:)` (`MangaQueries.swift:80`), `NovelQueries.insert(_:)` (`NovelQueries.swift:56`), `ChapterQueries.insert(_:)` single-row variant (`ChapterQueries.swift:61`), and `NovelQueries.upsertChapter(_:)` (`NovelQueries.swift:177`) all have real, working insert logic but zero call sites anywhere in `Yomi/`, joining the already-documented dead `delete`/`deleteAll` functions. Candidates for removal, or should be wired up if actually intended for a future feature. |
| 57 | `SourceBrowseView` reuses one cached `JSBridge` across concurrent `Task.detached` calls with no mutual exclusion | `BrowseView.swift:892` (`loadContent()`) and `:946` (`loadMore()`) both read the same `@State bridge` and each dispatch their own separate `Task.detached` (lines 913-935 and 957-984) with no guard preventing both from running against the same `JSContext` concurrently — `loadMore()` only checks `!isLoadingMore, hasMoreContent`, not whether `loadContent()`'s own task is still in flight. Self-documented hazard elsewhere in the same codebase: `NovelDetailView.swift:757-758` has a comment reading "Always resolve a fresh bridge — reusing a bridge from `SourceBrowseView` risks JSContext thread-safety issues when the context was last used on a different thread" — `SourceBrowseView` itself doesn't follow its own warning. |
| 58 | Two `JSBridge.isLNReaderPlugin` reads happen on MainActor, not inside `Task.detached` | `PopularSourceCarousel.load()` (`BrowseView.swift:462`) and `SourceBrowseView.loadContent()` (`BrowseView.swift:909`) both read `bridge.isLNReaderPlugin` before/outside their function's `Task.detached` block — violates the project's own absolute rule ("All JSBridge calls must be `Task.detached`... never call from MainActor") and `JSBridge.swift`'s own class comment: "JSContext is only ever accessed from background threads via `nonisolated` methods — never from the main actor." Two correctly-wrapped call sites in the same file (`BrowseView.swift:647`, `MigrateView.swift:270`) show the intended pattern, making these two the outliers. |
| 59 | `JSBridge.isPaperbackPlugin` is dead code | `JSBridge.swift:89` declares the property; no call site exists anywhere in the Swift codebase. |
| 60 | Migrating away from a source with "remove old entry" leaves old downloaded chapter files orphaned on disk | `MigrationService.swift:66-70`'s `if removeOld { ... }` block only flips `old.inLibrary = false` and upserts — no `FileManager` cleanup anywhere in the file, unlike the app's two other "remove from library" paths (`LibraryView.swift:505` and `MangaDetailView.swift:959`, both of which call `FileManager.default.removeItem(at:)` right alongside their own `inLibrary = false`). `DownloadsView.swift`'s download listing queries `DownloadQueries` directly, independent of `inLibrary`, so these orphaned files stay silently enumerated/counted with no UI path to ever clear them. |
| 61 | Hiding your current default tab via Customize Tabs can point cold-launch at a hidden tab | `CustomizeTabsView.swift:67-68`'s fallback for "you just hid the tab that's `AppSettings.defaultTab`" is `if settings.defaultTab == id.routerValue { settings.defaultTab = AppRouter.tabLibrary }` — a no-op exactly when the hidden tab *is* Library (the fresh-install default for `defaultTab`, and the only tab besides `.more` with no built-in hide protection — confirmed `AppSettings.swift:491,498`). Confirmed at the code level; the live consequence (cold launch lands `TabView(selection:)` on a tab id absent from the `ForEach`) follows from documented SwiftUI `TabView` selection semantics but wasn't reproduced live this session — flagging as plausible, not simulator-confirmed. |
| 62 | `ARQUITECTURA.md` states the wrong default accent color | `ARQUITECTURA.md:248` says `accentColor` defaults to `#FF6B6B`; the real default (`AppSettings.swift:453`) is `#E5473A` ("Vermilion" per `DesignTokens.swift:112-120`) — `#FF6B6B` is only the old "Coral" preset, not the default. The correct value is already stated elsewhere in this doc's own design-track summary, so this is a stale field description, not a project-wide misunderstanding. |
| 63 | `ARQUITECTURA.md`'s "Current tables" schema block predates v20/v21 | The `## Database` section header (`ARQUITECTURA.md:147`) still says "v19_source_indexes — last migration as of S77," and the SQL block under it (lines 148-197) shows `cloud_sync_map` with no `pendingChange` column and omits `pending_chapter_state`/`pending_novel_chapter_state` entirely — even though the same doc's own Migrations list (lines 220-224) correctly documents `v20_cloud_sync_map`/`v21_cloud_sync_pending` adding exactly those. Confirmed against `DatabaseManager.swift:253-297` that both really exist in the live schema. |
| 64 | `RESEARCH.md`'s "Last updated" banner predates its own S104 rewrite | `RESEARCH.md:2`'s banner still reads 2026-04-20 (S44), but this file's own S104 history states §5 (App Store Regulations) was substantively rewritten 2026-08-07 without the banner being bumped. Internally consistent with the S104 session record; exact line numbers/wording in `RESEARCH.md` itself weren't independently re-read this pass. |
| 65 | `TACHIMANGA_PARITY.md` has two stale line-number citations that no longer point at the code they describe | "Keep screen on while reading" (line 28) cites `ChapterReaderView.swift:136,143` for `isIdleTimerDisabled` — the real assignments are now at lines 192 and 199. "Per-card reading-status quick-set" (line 70) cites `LibraryView.swift:764` — that line is now an unrelated `.clipped()` modifier; the real "Reading Status" menu items are at lines 127, 245, and 861. Both features are still real/working — this is citation drift from code moving under the doc, not a content error. |
| 66 | `ARQUITECTURA.md`'s file-index entry for `YomiApp.swift` predates the S95 lock/onboarding merge | `ARQUITECTURA.md:111` still describes the pre-S95 pattern (`showOnboarding` gating its own standalone `.fullScreenCover`). The real code (`YomiApp.swift:181-203`, with an inline comment citing the same bug this doc's own Known Issue #14 describes) has one merged `.fullScreenCover` covering both `AppLockView` and `OnboardingView` with lock taking priority — the doc's file-index entry was never updated to match. |
| 67 | `ARQUITECTURA.md` undercounts `AppSettings`'s property count | `ARQUITECTURA.md:101` says "43 properties"; `AppSettings.swift` now has 55 persisted (`didSet { defaults.set... }`) properties. |
| 68 | Home Screen widget shows reading history even with App Lock / Secure Screen enabled | `WidgetDataWriter.write(_:)` (`WidgetDataWriter.swift:21-27`) is called unconditionally from `LibraryViewModel.writeWidgetData()` (`LibraryViewModel.swift:208-223`) on every library refresh, with no check of `AppSettings.appLockEnabled`/`secureScreenEnabled` (`AppSettings.swift:398,403`) anywhere in the call chain. Up to 5 items (titles + cover thumbnails) land in the shared App Group and render on `.systemSmall/.Medium/.Large` widgets (`YomiWidget.swift:217`) — visible on the Home Screen, and via the Lock Screen's Today View on default iOS settings, with no authentication, for an app whose own content is 18+ rated. |
| 69 | MangaDex plugin lists licensed/external chapters as normal ones, which dead-end at "No pages found" | `com.yomi.mangadex.js` (live production plugin, hosted at `yomi-plugins.web.app`, source lives outside this repo)'s `getChapterList` (lines 81-133) pushes every chapter from MangaDex's `/feed` response with no check of `attrs.pages`/`attrs.externalUrl`. Reproduced live via `curl` against MangaDex's real API: 5 of 6 recent One Piece English chapters have `pages: 0` and an `externalUrl` pointing at MangaPlus, and hitting `at-home/server/{chapterId}` for one of those returns HTTP 404. `getPageList` (lines 135-156) returns `[]` on that failure, which `ChapterReaderView.swift:478-481` surfaces as the generic "No pages found for this chapter." MangaDex is one of only 3 sources on the `instantInstallSourceIDs` one-tap-install allowlist (`PluginsView.swift:38-42`), so this is a first-run-friction bug for a source the app actively steers new users toward. |
| 70 | Reader's prev/next-chapter footer buttons have no `accessibilityLabel`, and the SF Symbols' default VoiceOver labels are backwards for their function | `ChapterReaderView.swift`: `Button(action: onPrevChapter)` (line 1270) wraps `Image(systemName: "backward.end.fill")` and `Button(action: onNextChapter)` (line 1277) wraps `"forward.end.fill"` — no `.accessibilityLabel` anywhere in the file. The exact VoiceOver-announced label text wasn't reproduced live this session (needs simulator accessibility-tree introspection), but the missing-label/wrong-icon-semantics code facts are solid. |
