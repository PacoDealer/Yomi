...
## Current state (post S117 — 2026-08-22 · S116 backlog fixed + remaining 15-dimension audit run, 30 new findings)
...
| 123 | Shikimori/Bangumi's OAuth Authorization Code Grant has no `state` param and no PKCE — a login-CSRF that can silently sync a victim's reading activity into an attacker's tracker account | **[HIGH/security — UNVERIFIED]** `Yomi/Features/More/ShikimoriService.swift:36` — the finder agent traced a concrete exploit chain (attacker obtains a valid `code` via their own OAuth flow against the same public `client_id`, gets the victim to open `yomi://shikimori/callback?code=<attacker's code>`, the app exchanges it and silently logs the victim's device into the attacker's account, then pushes real reading history into it) but the adversarial verify pass for this finding never ran before the account hit its rate limit again — **this needs independent confirmation before treating it as settled**, though the reasoning reads as sound and MAL's own PKCE `codeVerifier` check (cited as the working counterexample) is real. S117 audit workflow, found but not re-verified. |
| 124 | AniList's Implicit Grant `handleCallback` accepts any `access_token` in the redirect fragment with zero state/origin verification | **[HIGH/security — UNVERIFIED]** `Yomi/Features/More/AniListTrackerService.swift:48` — same class of finding as #123, same caveat: real-sounding, well-cited (traces the exploit through `ContentView`'s single `.onOpenURL` → `TrackerManager.route`), but never independently re-verified. Implicit Grant has no code-exchange step, so there's no PKCE fallback available here even in principle — `state` is the only defense the flow has, and it's absent. S117 audit workflow, found but not re-verified. |
| 125 | Chapter-update and reading-reminder push notifications show the real title on the lock screen regardless of App Lock/Secure Screen | **[MEDIUM/privacy — UNVERIFIED]** `Yomi/Core/NotificationManager.swift:39` — claimed to be the same threat model Known Issue #68 already fixed for the Home Screen widget, never extended to notifications. Not independently re-verified before the rate limit hit. S117 audit workflow, found but not re-verified. |
| 126 | `DownloadManager.performDownload()` mutates `@Observable` state after multiple await points with no MainActor hop — same bug class as #82, plus an inline comment claiming otherwise is wrong | **[MEDIUM/concurrency — UNVERIFIED]** `Yomi/Features/More/DownloadManager.swift:147` — cites a specific incorrect comment ("Update progress (already on MainActor — direct assignment)") at line 198. Not independently re-verified. S117 audit workflow, found but not re-verified. |
| 127 | `BackupManager` mutates `@Observable` iCloud/export state after real await points across every method, same bug class as #82 | **[MEDIUM/concurrency — UNVERIFIED]** `Yomi/Features/More/BackupManager.swift:162` — covers `exportBackup`/`uploadToICloud`/`downloadFromICloud`/`importBackup`. Not independently re-verified. S117 audit workflow, found but not re-verified. |
| 128 | All 4 tracker services mutate login state after `await URLSession.shared.data(for:)` with no MainActor hop, same bug class as #82 | **[MEDIUM/concurrency — UNVERIFIED]** `Yomi/Features/More/MALService.swift:63` — claimed present identically in MALService/AniListTrackerService/ShikimoriService/BangumiService. Not independently re-verified. S117 audit workflow, found but not re-verified. |
| 129 | The Home Screen widget's "last chapter" field is a hardcoded literal string ("Continue reading"), not the user's actual last-read chapter | **[MEDIUM/correctness — UNVERIFIED]** `Yomi/Features/Library/LibraryViewModel.swift:227` — `writeWidgetData()` allegedly never queries `ChapterQueries`/`NovelQueries` for the real chapter, contradicting the widget's own placeholder data (`"Chapter 1100"`). Not independently re-verified. S117 audit workflow, found but not re-verified. |
| 130 | All 15 production plugin `.js` files are bundled directly into the shipped binary (Debug and Release), contradicting CLAUDE.md's/ARQUITECTURA.md's own "App binary ships zero plugin files" claim | **[HIGH/app-store-compliance — UNVERIFIED]** `Yomi.xcodeproj/project.pbxproj:51` — claims a `PBXFileSystemSynchronizedBuildFileExceptionSet` explicitly lists all 15 plugins as bundle-resource exceptions, not Debug/Release-scoped, and that the only Swift code loading them by filename (`seedBundledPlugins()`) is `#if DEBUG`-gated — meaning all 15 ship in the Release/App-Store binary regardless, 7 of them unreferenced by any code at all. **This is the highest-stakes unverified finding — if real, it directly contradicts a compliance claim stated twice in this repo's own docs and deserves first priority to confirm.** Not independently re-verified before the rate limit hit. S117 audit workflow, found but not re-verified. |
| 131 | `SuwayomiService`'s detail/chapters/page-count REST methods have zero call sites — the whole REST surface for a Suwayomi manga's detail is unwired, and tapping into one may not be able to load chapters at all | **[MEDIUM/dead-code — UNVERIFIED]** `Yomi/Features/Extensions/SuwayomiService.swift:100` — claims `MangaDetailView` has no Suwayomi-specific handling and its `loadChapters()` guard always fails for a Suwayomi-sourced manga. If real this is a functional bug, not just dead code. Not independently re-verified. S117 audit workflow, found but not re-verified. |
| 132 | `MangaDetailView.formatReadingTime(_:)` is an orphaned duplicate helper, the same shape as the already-fixed Known Issue #104 in `NovelDetailView.swift` | **[LOW/dead-code — UNVERIFIED]** `Yomi/Features/Library/MangaDetailView.swift:971` — not independently re-verified. S117 audit workflow, found but not re-verified. |
| 133 | Three `Notation.swift` formatters (`volumeChapter`/`novelIndex`/`novelFooter`) have had zero call sites since S80 | **[LOW/dead-code — UNVERIFIED]** `Yomi/Core/Notation.swift:25` — not independently re-verified. S117 audit workflow, found but not re-verified. |
| 134 | `UpdatesViewModel`'s single-chapter mark-read methods have zero call sites — only the "mark all" siblings are wired to any UI | **[LOW/dead-code — UNVERIFIED]** `Yomi/Features/More/UpdatesView.swift:49` — not independently re-verified. S117 audit workflow, found but not re-verified. |
| 135 | `AniListTrackerService.redirectURI` is declared but never used, unlike the identical pattern in all 3 sibling tracker services | **[LOW/dead-code — UNVERIFIED]** `Yomi/Features/More/AniListTrackerService.swift:16` — not independently re-verified. S117 audit workflow, found but not re-verified. |
| 136 | `AdvancedSettingsView.showClearConfirm` is an unused `@State Bool` — the "Clear plugin catalog cache" button it was presumably meant to confirm fires with no confirmation dialog at all | **[LOW/dead-code — UNVERIFIED]** `Yomi/Features/More/AdvancedSettingsView.swift:11` — not independently re-verified. S117 audit workflow, found but not re-verified. |
| 137 | `MigrateView`'s `debounceTask` is an unused `@State Task?` — vestigial state from a debounce mechanism never implemented | **[LOW/dead-code — UNVERIFIED]** `Yomi/Features/Library/MigrateView.swift:69` — not independently re-verified. S117 audit workflow, found but not re-verified. |
