...
| 108 | ~~MAL/Shikimori/Bangumi store an OAuth `refresh_token` but never use it — tracker sync silently dies forever once the access token expires~~ | ✅ Fixed S120 — new `MangaTracker.refreshAccessToken()` protocol requirement (default `false`, correct for AniList's refresh-token-less Implicit Grant) implemented by all 3 code-grant services on top of a shared `performTokenRefresh(url:form:userAgent:)`. Every authenticated call now goes through a new `sendAuthorized(_:)` helper that refreshes and retries once on a 401 — `fetchUserInfo`/`searchManga` in all 4 services plus `sendProgressUpdate`. A refresh the server explicitly refuses (400/401) logs the tracker out and says "session expired — please log in again" instead of leaving the screen claiming "Connected"; a network failure is treated as transient and changes nothing. AniList's 401 (its ~1yr token expiring, no refresh possible) surfaces the same message. **Compile + review only** — a real round-trip needs the client credentials #115 notes are still missing. Original finding: **[HIGH/correctness]** `Yomi/Features/More/MALService.swift:88` — all three exchange the auth code for both `access_token` and `refresh_token`, save both to Keychain, but no function anywhere ever POSTs `grant_type=refresh_token` (confirmed via repo-wide grep — `grant_type` is only ever `"authorization_code"`). `isLoggedIn` is set once at login and never re-validated against the server. Once the short-lived access token actually expires, every `searchManga`/`updateMangaProgress` call from the reader's auto-update path silently no-ops via the existing `try?`/guard pattern — no error surfaces anywhere, and the Trackers screen keeps showing "Connected" indefinitely. AniList is unaffected (Implicit Grant, no refresh_token, ~1yr token life by design). S117 audit workflow, verified. |
...
| 114 | ~~A manga/novel that was opened and partially read (never reaching 80%/90% completion) never appears in History~~ | ✅ Fixed S120 — `ChapterQueries.updateProgress` now calls `MangaQueries.touchLastRead(mangaId:)` (it already resolves `mangaId` for its own dirty-mark, so no extra read), and `NovelQueries.updateScrollPercent` bumps the novel's `lastReadAt` via a new `touchLastReadIfStale(_:novelId:)` — **throttled to once a minute and folded into the existing write transaction**, because that path fires on every ~400ms scroll-autosave tick and an unconditional touch would have worsened #142; the CloudKit dirty-mark only fires when a row actually changed. **Live-verified**: opened a chapter, read to page 1 of 3, closed the reader — `chapter.progress` 0.333 with `isRead = 0` (the 80% path never ran) and `manga.lastReadAt` now set, and the History tab listed the title as "CH. 001 · read to 33%" where it previously showed nothing at all. The novel throttle's date-comparison SQL was verified directly against a real GRDB-written row (stale row updates, a row touched seconds earlier does not). Original finding: **[HIGH/correctness]** `Yomi/Database/Queries/ChapterQueries.swift:144` — `updateProgress(id:progress:readingSeconds:lastPageRead:)`, the function `ChapterReaderView`'s `.onDisappear` unconditionally calls on every reader close to persist resume position, never calls `MangaQueries.touchLastRead(mangaId:)` — only the ≥80%-completion `markChapterRead()` path does that. A manga opened and read a few pages of (well under 80%) correctly saves resume position but `Manga.lastReadAt` stays NULL forever, so it never shows up in `HistoryView` (which filters `WHERE lastReadAt IS NOT NULL`) despite being genuinely, recently read. The identical gap exists on the novel side (`NovelQueries.updateScrollPercent` also never touches `lastReadAt`). Distinct from the already-fixed Known Issue #20, which only covered the completion/manual mark-read call sites, not the ordinary in-progress save path. S117 audit workflow, verified. |
| 115 | ~~The entire Trackers feature never wires `\.yomiCanvas`~~ | ✅ Fixed S121 — all 5 screens (`TrackersView` + `MALView`/`AniListView`/`ShikimoriView`/`BangumiView`) now read `\.yomiCanvas`, paint their `List` with a new shared `.yomiListCanvas()` modifier, and use Space Grotesk/Space Mono + canvas ink for row text instead of system font + `.secondary`. **Live-verified on Paper**: Trackers and the MyAnimeList login screen both render on the warm cream canvas (previously cool system gray). Original finding: **[MEDIUM/design-system]** `Yomi/Features/More/TrackersView.swift:13` — `TrackersView`'s body is a bare `List { ... }.listStyle(.insetGrouped)` with zero `\.yomiCanvas` references, plain system font styling, and the 4 login-destination views (`MALView`/`AniListView`/`ShikimoriView`/`BangumiView`) are equally unstyled — unlike `SettingsView`/`StorageView`/`MoreView`/`DownloadsView`/`InsightsView`/`UpdatesView`, which all wire the root List's background to `canvas.bg` and rows to `canvas.surface1`/`surface2`. Most visible for Paper/Sepia users (cream palette everywhere else in the app vs. a cool system gray List background here). Built S115 in one session that consistently missed the canvas pass every other screen already got. S117 audit workflow, verified. |
| 116 | ~~`OPDSBrowseView` never wires `\.yomiCanvas`, and its 4 hand-rolled cover placeholders repeat the bug #38 already fixed~~ | ✅ Fixed S121 — the view and `OPDSItemDetailView` both read `\.yomiCanvas`; the two `List` paths use `.yomiListCanvas()` and the ScrollView/loading/error paths get `.background(canvas.bg.ignoresSafeArea())`; all 4 inline `Color.secondary.opacity(0.3)` placeholders became `canvas.surface2`, and every `.primary`/`.secondary` text became the canvas equivalent. **Live-verified on Paper against a local stand-in OPDS server** (~40-line Python, serving a nav root + a deliberately cover-less acquisition feed so the placeholder path actually renders) — the grid drew warm cream placeholders on the cream canvas. **Finding #163 was found doing this**: the screen was blank at HEAD for an unrelated reason. Original finding: **[MEDIUM/design-system]** `Yomi/Features/Browse/OPDSBrowseView.swift:168` — the whole file has zero `\.yomiCanvas`/`canvas.`/`.background(` occurrences, so this OPDS-catalog screen renders with plain system List/ScrollView backgrounds regardless of the selected canvas. Additionally, 4 separate hand-rolled no-cover placeholders (lines 168/195/202/269) use `Color.secondary.opacity(0.3)` instead of the canvas-aware `CoverImage` placeholder — the identical "ignores canvas selection" bug #38 fixed specifically in `Core/CoverImage.swift`, but that fix never reached this file's own inline placeholders. S117 audit workflow, verified. |
| 117 | ~~One icon-picker swatch in Appearance Studio is hardcoded `Color(.systemGray5)`~~ | ✅ Fixed S121 — `canvas.surface2` for the tile and `canvas.textSecondary` for its letter, matching every other surface in that file. Original finding: **[LOW/design-system]** `Yomi/Features/More/AppearanceStudioView.swift:391` — the alternate-icon swatch placeholder fill is a fixed system gray while the rest of this exact file (which reads `canvas` live via `settings.blendedCanvasColors`) consistently uses `canvas.surface1`/`canvas.bg` for every other surface. On Ink (dark warm bg) this one swatch renders as a light gray square clashing with every other tile on the same screen — the screen dedicated to demonstrating the app's own visual identity. S117 audit workflow, verified. |
| 118 | ~~`ChapterRow`/`ReadingStatusMenu` use system `Color.secondary` instead of `canvas.textSecondary`~~ | ✅ Fixed S121 — both structs now declare `@Environment(\.yomiCanvas)` and use `canvas.textSecondary`/`canvas.surface2`; the duplicate in `NovelDetailView` (which already had `canvas` in scope and still bypassed it) is fixed in the same pass. Original finding: **[LOW/design-system]** `Yomi/Features/Library/MangaDetailView.swift:1241` — neither struct declares `@Environment(\.yomiCanvas)`, so `Color.secondary` renders identically for Ink (warm cream-gray `textSecondary`) and Midnight (cool white `textSecondary`) even though the design system explicitly differentiates them and both canvases force `colorScheme = .dark`. The identical pattern is duplicated in `NovelDetailView.swift:620` — notably that file *does* have `\.yomiCanvas` in scope and still bypasses it. S117 audit workflow, verified. |
| 119 | ~~The scanlator filter-chip row hardcodes `Color.gray` for the unselected state~~ | ✅ Fixed S121 — both `.tint(...)` calls use `canvas.textSecondary` for the unselected state. Original finding: **[LOW/design-system]** `Yomi/Features/Library/MangaDetailView.swift:814` — `scanlatorChipRow` is a computed property on `MangaDetailView` itself (which has `@Environment(\.yomiCanvas) private var canvas` and uses it extensively elsewhere in the same file), but its `.tint(...)` calls at lines 814/821 hardcode `Color.gray` instead of a canvas-aware token like `canvas.textSecondary`. S117 audit workflow, verified. |
| 120 | ~~`YomiScrubber` has zero accessibility support — VoiceOver users cannot use it at all~~ | ✅ Fixed S121 — the view is now one `.accessibilityElement()` with a label, a spoken value and an `.accessibilityAdjustableAction` that steps by the scrubber's own `step` (clamped to `range`), i.e. everything a native `Slider` provides for free. Label and value formatting are per-call-site parameters, since neither is inferable from a bare number: the manga scrubber announces "Page — Page 3 of 20", the novel one "Font size — 18 points". **Compile + review only** — reaching either scrubber live needs a chapter that actually loads pages (the manga scrubber only renders at `totalPages > 1`), which this session couldn't produce; see #122 for the same overlay's labels verified live in the real accessibility tree. Original finding: **[HIGH/accessibility]** `Yomi/Core/YomiScrubber.swift:10` — a fully custom `GeometryReader`+`Capsule`/`Circle` slider driven purely by `DragGesture`, with no `.accessibilityElement()`, `.accessibilityLabel`, `.accessibilityValue`, or `.accessibilityAdjustableAction` (a native SwiftUI `Slider` gets all of this for free). Used at `ChapterReaderView.swift:1303` (manga page scrubber — the only way to jump to an arbitrary page) and `TextReaderView.swift:824` (novel font-size control) — both core reading-adjustment controls with no accessible alternative anywhere in the UI. S117 audit workflow, verified. |
| 121 | ~~`TextReaderView`'s entire reader overlay has zero `accessibilityLabel` calls~~ | ✅ Fixed S121 — labels added to dismiss ("Close reader"), chapters, view-source, prev/next chapter and the TTS toggle (which now announces its state through the label itself: "Read aloud" / "Stop reading aloud"). The controls whose selection was conveyed only by colour or a glyph — the serif-font and justify toggles, the 5 theme swatches, and the margin/line-spacing segmented rows — also got `.accessibilityValue`/`.isSelected` so the *current* setting is announced, not just the control's name. The two segmented rows were beyond the filed finding but are the same defect ("Narrow" alone never said narrow *what*). **Compile + review only** — the novel reader wasn't reachable live this session (the seeded novel's source is one of the dead ones, and the library cell hit the documented mobile-mcp tap flakiness). Original finding: **[MEDIUM/accessibility]** `Yomi/Features/Reader/TextReaderView.swift:717` — `grep -n "accessibilityLabel" TextReaderView.swift` returns zero matches. Unlabeled: dismiss, open-chapters, view-source, font-family toggle, justify-text toggle, the 5 novel-theme swatch circles (color-only selection, same problem class as the accent picker), prev/next-chapter buttons, and the TTS play/pause toggle (never announces speaking state). Known Issue #70 already fixed the identical bug class for the *manga* reader's footer buttons — that fix never touched this sibling file. S117 audit workflow, verified. |
| 122 | ~~The manga reader overlay's top-bar icon buttons have no accessibility labels~~ | ✅ Fixed S121 — "Close reader", "Chapters", "Open on source website", "Reader settings". **Live-verified in the real accessibility tree**: seeded a library manga + chapter via direct `sqlite3`, opened the reader, and `mobile_list_elements_on_screen` reported those labels where it previously returned the raw SF Symbol names (`chevron.left`/`list.bullet`/`gearshape`) — the same tree that already showed #70's footer labels correctly, which is what confirms the modifier reaches VoiceOver here. (The seed initially looked like a dead chapter row: its `sourceId` was `mangadex` while the installed extension id is `com.yomi.mangadex`, so `canOpenReader` was correctly false — the third cause of a dead-looking button, per S120.) Test rows removed afterward. Original finding: **[MEDIUM/accessibility]** `Yomi/Features/Reader/ChapterReaderView.swift:1138` — `ReaderOverlayView`'s top bar has 4 icon-only buttons (dismiss/chapters/source/settings) with no `.accessibilityLabel`, unlike the footer's prev/next-chapter buttons which #70 already labeled. VoiceOver reads the raw SF Symbol name ("chevron left") instead of anything meaningful ("Close reader"). S117 audit workflow, verified. |
| 123 | ~~Shikimori/Bangumi's OAuth Authorization Code Grant has no `state` param and no PKCE — a login-CSRF~~ | ✅ Fixed S120 — new shared `MangaTracker.makeAuthState()`/`verifyAuthState(url:requireEcho:)` protocol extension: every tracker now sends an unguessable 256-bit `state` (via `SecRandomCopyBytes`) built when *this app* constructs the authorization URL, stored in a new `pendingAuthState` protocol requirement, and consumed single-use on callback. Shikimori/Bangumi require the echo to match exactly; an unsolicited callback is rejected with a visible `errorMessage`. MAL's own hardcoded `state: "yomi"` was replaced by the same per-attempt value (it had PKCE, but a constant `state` is not a check). Original finding: **[HIGH/security]** `Yomi/Features/More/ShikimoriService.swift:36` — ✅ verified S118 by reading `ShikimoriService.swift` directly: `authorizationURL()` sends no `state` param, `handleCallback` only extracts `code` from the URL with zero verification it was solicited by this app instance. Confirmed by contrast against `MALService.swift`, the cited working counterexample: MAL's `handleCallback` requires a locally-generated `codeVerifier` (stored in memory when the app itself opened the auth URL) to exchange the code — an attacker's own `code` fails that exchange. Shikimori/Bangumi have no equivalent check. Real exploit: attacker gets a valid `code` via their own login against the app's public `client_id`, gets victim to open `yomi://shikimori/callback?code=<attacker's code>`, app silently logs the victim's device into the attacker's account and pushes real reading history into it. Not yet fixed. |
| 124 | ~~AniList's Implicit Grant `handleCallback` accepts any `access_token` in the redirect fragment with zero state/origin verification~~ | ✅ Fixed S120 alongside #123 — AniList now sends `state` (and `redirect_uri`, which also closes #135) and verifies the callback with `requireEcho: false`: the fragment's `state` must match when present, and a callback arriving with **no login pending on this device** is rejected outright, which is the actual attack shape. The looser echo rule is deliberate — AniList's docs don't promise the implicit-grant fragment carries `state` back, and failing closed on that would break login against a live server this session couldn't test. Original finding: **[HIGH/security]** `Yomi/Features/More/AniListTrackerService.swift:48` — ✅ verified S118 by reading `AniListTrackerService.swift` directly: `authorizationURL()` sends no `state`, `handleCallback` extracts `access_token` from the URL fragment and trusts it unconditionally. Implicit Grant has no code-exchange step, so there's no PKCE fallback available here even in principle — `state` is the only defense the flow has, and it's absent. Same exploit shape as #123: an attacker-obtained token delivered via `yomi://anilist/callback#access_token=...` silently logs the victim into the attacker's AniList account. Not yet fixed. |
| 125 | ~~Chapter-update and reading-reminder push notifications show the real title on the lock screen regardless of App Lock/Secure Screen~~ | ✅ Fixed S120 — new `NotificationManager.shouldHideTitles` (`appLockEnabled || secureScreenEnabled`, the same guard #68 uses for the widget): a chapter notification's title becomes "Yomi" and the reading reminder falls back to "Your reading list is waiting." The notification is still sent and still deep-links correctly (behind App Lock) — the title is withheld, not the signal. Both call sites are already on MainActor, so reading `AppSettings.shared` here doesn't violate the project's detached-task rule. Reminders are re-scheduled (title re-evaluated) on every backgrounding, which is the same action that must happen before any pending one could be seen. Compile + review only — triggering a real chapter notification needs an actual upstream update. Original finding: **[MEDIUM/privacy]** `Yomi/Core/NotificationManager.swift:39` — ✅ verified S118 by reading `NotificationManager.swift` + both call sites (`UpdatesView.swift:206,281`, `YomiApp.swift:242`): `scheduleChapterNotification`/`scheduleReadingReminder` both put the real manga/novel title directly into `content.title`/`content.body` with zero `AppSettings.shared.appLockEnabled`/`secureScreenEnabled` check anywhere in the call chain — same threat model Known Issue #68 fixed for the Home Screen widget, never extended here. Not yet fixed. |
...
| 129 | ~~The Home Screen widget's "last chapter" field is a hardcoded literal string~~ | ✅ Fixed S120 — new bulk `ChapterQueries.fetchLastTouchedChapterNames(mangaIds:)` / `NovelQueries.fetchLastTouchedChapterNames(novelIds:)` (one windowed SQL query each, only for the ≤5 ids the widget actually shows — deliberately not the N+1 shape of #140) resolve the most recently read-or-started chapter per title; `writeWidgetData()` is now `async` and runs them in `Task.detached`. Falls back to the old literal only when a title has no chapter records at all. **Live-verified**: seeded a library manga with one read chapter, relaunched, and the App Group plist's `widgetReadingItems` showed `"lastChapter": "Ch. 01"` — the real chapter name. Original finding: **[MEDIUM/correctness]** `Yomi/Features/Library/LibraryViewModel.swift:227` — ✅ verified S118 by reading `writeWidgetData()` directly: line 227 hardcodes `lastChapter: "Continue reading"` for every entry — never queries `ChapterQueries`/`NovelQueries` for the real last-read chapter, contradicting the widget's own placeholder data (`"Chapter 1100"`). Not yet fixed. |
...
| 131 | ~~`SuwayomiService`'s detail/chapters/page-count REST methods have zero call sites — a Suwayomi manga's chapters can never load from `MangaDetailView`~~ | ✅ Fixed S120 — `MangaDetailView.loadChapters()` now branches on `SuwayomiService.isSuwayomiSourceId(sourceId)` into a new `loadSuwayomiChapters()` that fetches detail + chapters over REST, fills in summary/author/genres/status, persists and merges local read state, and sorts ascending (#50's convention). Chapters carry a self-describing `suwayomi://{mangaId}/{chapterIndex}` path; `ChapterReaderView.bridge` became optional and a new `fetchPages(bridge:path:)` resolves pages either through the JS plugin or through `SuwayomiService.fetchPageURLs`. **Two further `bridge != nil` gates were the reason the fix wasn't enough on its own** — they hid the Start-reading button and made every chapter row's tap a no-op; both now use a `canOpenReader` computed property. A new `chapterLoadError` surfaces a real REST failure instead of a blank list. **Live-verified end to end** against a local stand-in server speaking the same REST shapes (no Docker/JDK 21 on this machine for a real Suwayomi): Browse → Stub Source → detail rendered author/genres/status/synopsis + all 5 chapters → Start reading → the reader fetched `chapter/1?onlineFetch=true` and all 3 page images and rendered them. Original finding: **[HIGH/correctness]** `Yomi/Features/Extensions/SuwayomiService.swift:100` — ✅ verified S118, and worse than filed (upgraded from MEDIUM/dead-code to HIGH/correctness — a real functional bug, not just dead code). `fetchMangaDetail`/`fetchChapters(mangaId:)`/`fetchChapterPageCount`/`suwayomiMangaId(from:)` have zero call sites anywhere outside their own file (confirmed by grep). `SuwayomiBrowseView.swift:95` navigates a tapped Suwayomi manga to the shared `MangaDetailView`, which has zero `Suwayomi` references and no Suwayomi-specific handling anywhere. `SuwayomiService.toManga(item:sourceId:)` sets `sourceId: "suwayomi_\(sourceId)"` (line 130) — this can never match an entry in `ExtensionManager.shared.installed` (real JS-plugin ids like `"mangadex"` or sha256 hashes), so `MangaDetailView.loadChapters()`'s guard (`ExtensionManager.shared.installed.first(where: { $0.id == sourceId })`, line 998) always fails for a Suwayomi-sourced manga, falling into the "extension not installed" branch that only shows whatever's already cached in the local DB (empty for any manga not yet added to the library, since `toManga` sets `inLibrary: false`). Concrete effect: tapping any Suwayomi manga from Browse → detail shows no chapters, ever, with no error surfaced. Not yet fixed. |
| 132 | ~~`MangaDetailView.formatReadingTime(_:)` is an orphaned duplicate helper~~ | ✅ Fixed S120 — removed (zero call sites re-confirmed by grep first), same as #104's fix in the sibling file. Original finding: **[LOW/dead-code]** `Yomi/Features/Library/MangaDetailView.swift:971` — ✅ verified S118 — zero call sites in the file besides its own definition. Not yet fixed. |
| 133 | ~~Three `Notation.swift` formatters (`volumeChapter`/`novelIndex`/`novelFooter`) have had zero call sites since S80~~ | ✅ Fixed S120 — all three removed after re-confirming zero call sites repo-wide. Original finding: **[LOW/dead-code]** `Yomi/Core/Notation.swift:25` — ✅ verified S118 — repo-wide grep finds each name only at its own declaration, no Swift call sites anywhere. Not yet fixed. |
| 134 | ~~`UpdatesViewModel`'s single-chapter mark-read methods have zero call sites~~ | ✅ Fixed S120 — `markMangaChapterRead`/`markNovelChapterRead` removed; the `markAll*` siblings the UI actually calls are untouched. Original finding: **[LOW/dead-code]** `Yomi/Features/More/UpdatesView.swift:49` — ✅ verified S118 — `markAllMangaChaptersRead`/`markAllNovelChaptersRead` are called from the view body (lines 489/506), `markMangaChapterRead`/`markNovelChapterRead` (single-chapter siblings, lines 49/64) have zero call sites anywhere. Not yet fixed. |
| 135 | ~~`AniListTrackerService.redirectURI` is declared but never used~~ | ✅ Fixed S120 — `authorizationURL()` now passes it as `redirect_uri`, matching all 3 sibling services. It must match the redirect URL registered for the client on anilist.co (`yomi://anilist/callback`, as documented in `AppSecrets.swift.template`). Original finding: **[LOW/dead-code]** `Yomi/Features/More/AniListTrackerService.swift:16` — ✅ verified S118 — appears only at its own declaration in the file; MAL/Shikimori/Bangumi all pass their `redirectURI` into `redirect_uri` request params, AniList's Implicit Grant `authorizationURL()`/`handleCallback` never reference it. Not yet fixed. |
| 136 | ~~`AdvancedSettingsView.showClearConfirm` is an unused `@State Bool`~~ | ✅ Fixed S120 by removing the dead state, **not** by adding the dialog it implies: `PluginCatalogService.invalidateCache()` destroys nothing a refetch doesn't restore, so a confirmation would be friction on a harmless, instant action. Original finding: **[LOW/dead-code]** `Yomi/Features/More/AdvancedSettingsView.swift:11` — ✅ verified S118 — the button (line 32) calls `PluginCatalogService.shared.invalidateCache()` directly with no `.confirmationDialog`/`.alert` anywhere in the file referencing `showClearConfirm`. Not yet fixed. |
| 137 | ~~`MigrateView`'s `debounceTask` is an unused `@State Task?`~~ | ✅ Fixed S120 — removed. (`BrowseView` has a same-named property that *is* live — checked before deleting.) Original finding: **[LOW/dead-code]** `Yomi/Features/Library/MigrateView.swift:69` — ✅ verified S118 — appears only at its own declaration, never assigned or cancelled anywhere else in the file. Not yet fixed. |
...
| 139 | ~~`BackupManager.buildBackupData()` synchronously JSON-encodes the entire library on the main thread~~ | ✅ Fixed S120 — the encode + `JSONSerialization` step now runs in `Task.detached`; the 5 `encode*` helpers became `nonisolated static` (matching their existing `decode*` siblings) so they're callable from there. `.prettyPrinted` dropped — nothing but `decodeBackup` ever reads this file. Original finding: **[MEDIUM/performance]** `Yomi/Features/More/BackupManager.swift:129-157` — verified S118. DB reads correctly use `await appDatabase.read`, but the subsequent `mangas.map { encodeManga($0) }` etc. + `JSONSerialization.data(withJSONObject:options: .prettyPrinted)` run with no `Task.detached`, directly on MainActor (same isolation reasoning as #138). Called from `exportBackup()` and `uploadToICloud()`, both `await`ing it directly on MainActor before the file-write step (which IS correctly detached). `.prettyPrinted` adds needless overhead for a machine-only backup format. Causes a visible freeze on "Export Backup"/"Upload to iCloud" for a large library. Not yet fixed. |
| 140 | ~~`BackupManager.exportTachiyomiBackup()` does one DB round-trip per library manga instead of one bulk query, on the main thread~~ | ✅ Fixed S120 — new `ChapterQueries.fetchAllGrouped(mangaIds:)` (one `WHERE mangaId IN (…)` query, grouped client-side) replaces the per-manga loop, and both it and the library fetch moved into `Task.detached`. Original finding: **[MEDIUM/performance]** `Yomi/Features/More/BackupManager.swift:110-116` — verified S118: `for manga in mangas { chaptersByMangaId[manga.id] = try ChapterQueries.fetchAll(mangaId: manga.id) }`, no `Task.detached` anywhere in the function, so it runs on MainActor. N+1 pattern — for a 200-manga library, 200 sequential synchronous DB reads instead of one bulk `SELECT ... WHERE mangaId IN (...)` grouped client-side. Not yet fixed. |
| 141 | ~~`UpdatesView`'s "mark all read" issues 4 separate write transactions per chapter~~ | ✅ Fixed S120 — new `ChapterQueries.setReadBatch(chapterIds:mangaId:isRead:)` and `NovelQueries.markReadBatch(chapterIds:novelId:)`: one `UPDATE … WHERE id IN (…)`, one `markCloudDirtyBatch`, one `touchLastRead` for the whole list. `markAllRead(mangaId:)` couldn't be reused directly because this path marks a *specific* set of ids, not every chapter of the title. Original finding: **[MEDIUM/performance]** `Yomi/Features/More/UpdatesView.swift:57-61,72-76` — verified S118. `markAllMangaChaptersRead`/`markAllNovelChaptersRead` loop `ids.forEach { ChapterQueries.setRead(...) }`; `setRead` (`ChapterQueries.swift:110-127`) does its own `appDatabase.write` (UPDATE) + `markCloudDirty` (its own write) + `MangaQueries.touchLastRead` (its own write + its own `markCloudDirty` write) = 4 writes per chapter. The codebase already has the correct batched pattern one function over — `ChapterQueries.markAllRead(mangaId:)` (`ChapterQueries.swift:131-141`) does one bulk `UPDATE ... WHERE mangaId = ?` + one `markCloudDirtyBatch` call — but `UpdatesView`'s "mark all read for these specific ids" path never uses it. Concrete scenario: tapping "Mark all read" on a series with 20+ piled-up chapters issues 80+ sequential write transactions where ~3 would do. Not yet fixed. |
| 142 | ~~Novel reader's scroll-position autosave issues 2-3 separate write transactions per debounced scroll pause, with no coalescing~~ | ✅ Fixed S120 — `onScrollUpdate` now skips any tick that hasn't moved ≥1% from the last value actually persisted (new `lastPersistedScrollPercent`, reset on chapter change). Nothing is lost: `flushScrollPercent()` on `.onDisappear` and on chapter navigation always writes the final position. Complements #114's throttled `lastReadAt` touch in the same path. Original finding: **[MEDIUM/performance]** `Yomi/Features/Reader/TextReaderView.swift:206-213` — verified S118. `onScrollUpdate` fires `NovelQueries.updateScrollPercent(chapterId:percent:)` inside `Task.detached(priority: .background)` on every JS-side 400ms debounce tick. `updateScrollPercent` (`NovelQueries.swift:212-223`) does one `appDatabase.write` (UPDATE + a SELECT novelId) then `markCloudDirty` — itself `rememberMapping`'s own write, plus a third `markPending` write if `cloudSyncEngine` isn't running yet. Fires repeatedly throughout a reading session with no in-app coalescing or "already dirty" check, unlike the manga reader (`ChapterReaderView.swift`), which only persists progress once on `.onDisappear`. Runs in the background (doesn't freeze the UI) but is a real, avoidable multiplication of write I/O. Not yet fixed. |
| 143 | ~~`MigrationService.migrate` does an O(n·m) linear chapter-number match plus up to 2 separate write transactions per matched chapter~~ | ✅ Fixed S120 — builds a `[Double: Chapter]` index once instead of re-scanning per old chapter, and collects the matches into two batched calls (`setReadBatch` + a new `updateProgressBatch`) instead of up to two transactions per chapter. Original finding: **[MEDIUM/performance]** `Yomi/Features/Library/MigrationService.swift:44-62` — verified S118: `freshNewChapters.first(where: { $0.chapterNumber == oldNum })` runs inside `for oldCh in readOldChapters`, a linear scan repeated per already-read old chapter instead of building a `[Double: Chapter]` dictionary once. Each matched chapter can trigger `setRead` (see #141's write cascade) plus a separate `updateProgress` call. Concrete scenario: migrating a completed, fully-read 1000-chapter series issues on the order of 2,000-3,000 individual serialized write transactions. Runs inside `Task.detached` at the call site (`MigrateView.swift:295-296`) so the UI doesn't freeze, but the "Migrating…" spinner runs materially longer than necessary. Not yet fixed. |
| 144 | ~~`novel_chapter` has no index on `isRead` — `fetchUnreadCountsByNovel()` does a full table scan on every Library load~~ | ✅ Fixed S120 — new migration `v22_novel_chapter_unread_index` adds `idx_novel_chapter_unread` on `novel_chapter(novelId, isRead)`. **Next migration prefix is now `v23_`.** **Live-verified**: migration applied cleanly to the existing simulator DB, and `EXPLAIN QUERY PLAN` for the exact `fetchUnreadCountsByNovel` query now reads `SCAN novel_chapter USING COVERING INDEX idx_novel_chapter_unread` (was a full table scan). Original finding: **[LOW/performance]** `Yomi/Database/DatabaseManager.swift` (v18_indexes migration) — verified S118: `idx_chapter_unread` exists on `chapter(mangaId, isRead)` but no equivalent index exists on `novel_chapter`, confirmed by reading the full index list in `DatabaseManager.swift`'s migrations (`idx_chapter_mangaid`, `idx_chapter_unread`, `idx_novel_chapter_novelid`, `idx_manga_sourceid`, `idx_novel_sourceid` — none touch `novel_chapter.isRead`). `NovelQueries.fetchUnreadCountsByNovel()` (`NovelQueries.swift:135-147`) does `SELECT novelId, COUNT(*) FROM novel_chapter WHERE isRead = 0 GROUP BY novelId`, run on every `LibraryViewModel.loadLibrary()` call. Direct violation of this repo's own ABSOLUTE RULE ("every new `WHERE column = ?` query pattern on a large table needs a corresponding index"). Fixable with a new `v22_` migration adding `idx_novel_chapter_unread` on `novel_chapter(novelId, isRead)`. Not yet fixed. |
...
| 146 | ~~`BrowseView`'s Suwayomi/OPDS root-list load failures are indistinguishable from "nothing configured yet"~~ | ✅ Fixed S120 — new `suwayomiError`/`opdsError` state set in every `catch`; each section's empty state now shows "Couldn't reach the Suwayomi/OPDS server — <reason>" with the button relabelled **Try again**. A failed "Load More" reports into the same slot rather than replacing already-loaded content. **Live-verified**: pointed `suwayomiURL` at a dead port, and Browse read "Couldn't reach the Suwayomi server — Could not connect to the server." instead of a bare "Load sources". Original finding: **[MEDIUM/error-handling]** `Yomi/Features/Browse/BrowseView.swift:286-327` — verified S118: `loadSuwayomiSources()`/`loadOPDSRoot()`/`loadMoreOPDSRoot()`'s `catch` blocks only reset the loading flag (`await MainActor.run { suwayomiLoading = false }` etc.), never set any error state. Contrast with the sibling `OPDSBrowseView.swift:231-233`'s `catch` on the identical kind of fetch, which does set `errorMessage = error.localizedDescription`. A user whose self-hosted Suwayomi/OPDS server is unreachable gets the same "Load sources"/"Load library" empty-state button shown when the feature has simply never been loaded, with zero indication why. Not yet fixed. |
...
| 150 | ~~`LibraryViewModel.errorMessage` is set on a DB-read failure but never read/displayed anywhere — the Library tab just looks empty~~ | ✅ Fixed S120 — `LibraryView`'s empty-state branch now checks `viewModel.errorMessage` first and renders a "Couldn't load your library" state with the real error and a **Try again** button that re-runs `loadLibrary()`. The novel half of the fetch also moved from `try?` to the same `do/catch` — half a library rendering as the whole one is the same silent failure; unread counts stay best-effort, since a missing badge isn't a lie about what's in the library. **Live-verified**: renamed the `manga` table in the simulator's `yomi.db`, relaunched, and the screen read "SQLite error 1: no such table: manga…" instead of "Your library is empty"; restored the table and Try again recovered cleanly. Original finding: **[MEDIUM/error-handling]** `Yomi/Features/Library/LibraryViewModel.swift:184-203` — verified S118: `loadLibrary()`'s `catch` branch sets `errorMessage = error.localizedDescription` (via the `result.4` tuple slot), but `grep -n "errorMessage"` across `LibraryView.swift` returns zero matches — the property is written but never read, and the file has zero `.yomiToast`/`YomiToast` usage (confirmed by grep), unlike the already-fixed #26 paths. If `MangaQueries.fetchLibrary()` throws (DB corruption, disk I/O error), the app's primary screen silently renders its normal empty-library state, indistinguishable from "you haven't added anything yet." Not yet fixed. |
...
| 163 | ~~Drilling into any OPDS navigation entry showed a permanently blank screen and never fetched the feed~~ | ✅ **Found and fixed S121** while live-verifying #116 — **not** a regression from that work: reproduced identically on an unmodified HEAD build (`git stash`ed the design changes, rebuilt, same blank screen, same zero requests). **[HIGH/correctness]** `Yomi/Features/Browse/OPDSBrowseView.swift:23` — `body` was a `Group` whose three branches are `isLoading` / `errorMessage != nil` / `feed != nil`, **all false in the initial state** (`isLoading` starts `false`, `feed` starts `nil`). A SwiftUI `Group` distributes its modifiers to its children, so with zero children the `.task { await load() }`, `.navigationTitle` and `.sheet` attached to nothing — `load()` never ran, nothing ever set `isLoading`, and the Group stayed empty forever. Self-sustaining deadlock: the only thing that could have populated a branch was the very modifier that never attached. Fixed by giving the `else` a non-empty `Color.clear`. **Live-verified against a local stand-in OPDS server**: before, tapping an OPDS folder gave a blank screen with no title and the server logged **zero** requests; after, `GET /books` fires and the grid renders with its title. This means OPDS drill-down — the whole point of the OPDS feature beyond the root list — has almost certainly never worked; `BrowseView` fetches the root itself via its own `loadOPDSRoot()`, which is why the root list always looked fine. Not covered by the S116-S118 audit, which never drilled into an OPDS feed against a live server. |
