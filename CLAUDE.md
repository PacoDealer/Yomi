...
## Current state (post S110 — 2026-08-17 · Customize Tabs fully verified; found+fixed a real chapter-ordering bug; boundary-preload trigger still unconfirmed)
...
| 51 | Chapter-boundary preload's trigger couldn't be confirmed firing in the simulator | Found S110 — after fixing #50, seeded a chapter to resume 2-3 pages from the end and confirmed via Pulse's Network Console (cross-checked directly against the simulator's Pulse `logs.sqlite`, not just the in-app UI) that the next chapter's page-list request **never fires**, across 3 separate app-relaunch attempts and both a direct-seek and an organic multi-swipe approach to the documented `idx >= pages.count - 3` threshold. Temporary `NSLog` instrumentation (added then removed, not committed) confirmed `WebtoonReaderView`'s `.onChange(of: visibleId)` never fires at all during `mobile-mcp` swipes in this run, despite content visibly scrolling on screen — meaning `currentPage` isn't updating internally either. Suspected cause: `WebtoonReaderView` mixes the older `ScrollViewReader.scrollTo` API (used to jump to a saved resume page) with the newer `.scrollPosition(id:)` reactive binding (used to detect the currently-visible page) on the *same* `ScrollView` — an undocumented combination that may not propagate scroll changes from certain gesture sources into the binding. Not ruled out: a genuine code bug independent of tooling. **Next session should verify directly on Martin's real device** (genuine touch-drag, not synthetic) before assuming this is tooling-only; if the card still doesn't appear there, re-add similar `NSLog`/breakpoint instrumentation with real touch input. |
