...
|---|---|---|---|
| Reading modes | 7: Paged LTR/RTL/vertical, Continuous LTR/RTL/vertical, Webtoon | 3: `ReaderMode` enum has only `horizontalRTL` ("Manga RTL"), `horizontalLTR` ("Manhwa LTR"), `verticalScroll` ("Webtoon") — `Yomi/Features/Reader/ChapterReaderView.swift:6-10` | ❌ Missing: paged-vertical, continuous-LTR, continuous-RTL, continuous-vertical (non-webtoon) |
...
| Cloudflare bypass | Automatic, global toggle, configurable timeout, request-dump debug log, applies to all source types uniformly | `CFBypassView.swift` exists but is wired only into `MangaDetailView.swift` (manga path). `NovelDetailView.swift` has **zero** CFBypass references — confirmed via grep. This is the still-open Known Issue #11 (FreeWebNovel and others blocked for novels). | ❌ Real, confirmed gap — highest-value fix, not just a Tachimanga-parity nicety |
