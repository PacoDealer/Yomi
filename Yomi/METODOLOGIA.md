...
| 44-pre | 2026-04-20 | New user onboarding + multi-format catalog fix + research. (1) `PluginsView.swift` rewrite: toolbar `+` → `Menu` → "Add Repository" opens `AddRepoSheet` (featured repos + custom URL field + GitHub guide link); installed-section empty state shows featured repos + guide link; `FeaturedRepo` struct with one-tap `Add` button that appends to `pluginCatalogURLs` + triggers refresh. (2) `PluginCatalogService.swift`: multi-format parser — tries Yomi native JSON, falls back to LNReader format (`lang`/`url`/`iconUrl` → `language`/`fileURL`/`iconURL`); `description` made optional; `withThrowingTaskGroup` → `withTaskGroup` (per-URL silent failure, errorMessage only set if ALL fail). `nonisolated` on `parseEntries` + `LNReaderEntry.toEntry()` (Swift 6 @Observable isolation). (3) `SettingsView.swift`: added GitHub guide `Link` to add-repo sheet. (4) `README.md`: new file — Quick Start + 3-repo comparison table (Yomi Catalog/LNReader Novels/Keiyoushi) + Tachiyomi migration section. Suwayomi self-hosting instructions removed (usability research: too complex). (5) Research: Tachimanga architecture CORRECTED — NOT Flutter+JVM, actual architecture = Flutter frontend + C-native DEX bytecode interpreter (no JIT — App Store allows interpretation). This means Keiyoushi APKs run via interpreted DEX bytecode, no embedded JVM. (6) Mihon forks documented: TachiyomiJ2K (~5.2k, dual-page), TachiyomiSY (~3.8k, enhanced tracking), TachiyomiAZ (legacy design), Yōkai (~1.7k, J2K+), Komikku (~3.7k, auto-webtoon) — all Android-only, no new iOS paths. All use Keiyoushi APK system. RESEARCH.md updated: sections 7b (Tachimanga DEX correction), 16 (Mihon forks), 17 (catalog multi-format support), 18 (three-repo strategy). |
