...
| 45 | 2026-04-23 | Cloudflare auto-bypass + LNReader/Mangayomi plugin fixes. (1) `CFBypassView.swift`: manual full browser sheet (unchanged from S45 part A). (2) `CFBypassManager` enum + `AutoBypassHelper` class: hidden 1×1pt `WKWebView` added to `keyWindow`, polls `WKHTTPCookieStore` every 0.5s for `cf_clearance`, resolves `withCheckedContinuation<Bool>` on success or 10s timeout. (3) `JSBridge.swift`: `injectSourceFetch` captures `ObjectIdentifier(ctx)` as `ctxID` before the `@convention(block)` — detects CF responses via `CF-RAY` header or (403 + "Just a moment"/"cf-mitigated" in body) — stores `cfBlockedURL` in module-level `_cfBlockedByContext` dict (guarded by `NSLock`). `cfBlockedURL`, `clearCFBlock()`, `deinit` added to JSBridge instance. (4) `SourceBrowseView.loadWithBypass()`: wraps `loadContent()` — on empty result checks `bridge.cfBlockedURL`, auto-triggers `CFBypassManager.autoBypass(url:)`, shows `isBypassing` overlay, retries on success. (5) `injectLNReaderAdapter` JS: checks `module.exports`/`exports.default` as fallback when `globalThis.plugin` is unset — fixes LNReader v3.0.0 plugins (DaoNovel etc.) that export via CommonJS. (6) `PluginCatalogService.parseEntries`: filters Mangayomi entries to `.js` suffix only — prevents Dart extensions from appearing in catalog. Cleanup: removed dead `YomiCatalogEntryRow` from `PluginsView.swift`. |
