...
| 19 | 2026-04-06 | AppSettings: added colorScheme: ColorScheme? computed var + hasSeenOnboarding: Bool UserDefaults property. YomiApp: preferredColorScheme at WindowGroup root, removed seedBundledPlugins() call, added fullScreenCover(isPresented: $showOnboarding). OnboardingView: 3-page TabView(.page) on #1C1C1E — WelcomePage (book.fill), InstallPluginPage (puzzlepiece.extension.fill), ReadyPage (checkmark.circle.fill → tabMore). ChapterReaderView: Color.clear tap layer behind reader content for immersive toggle. TextReaderView: dark #1C1C1E/#E8E8E8, sepia #FFF8F0/#2C1810, light #FFFFFF/#1C1C1E, line-height 1.6. HistoryView: plugin display name lookup via ExtensionManager.shared.installed. |

## S19 — Technical learnings

- **preferredColorScheme at WindowGroup root**: apply `.preferredColorScheme()` once on `ContentView()` inside `WindowGroup` in `YomiApp.swift`. Never apply it on individual views or NavigationStacks — it must own the entire window to work correctly. Child views that had `.preferredColorScheme(.dark)` hardcoded (TextReaderView, ChapterReaderView) must have that modifier removed.

- **AppSettings.colorScheme computed var**: store theme as a `String` ("System"/"Light"/"Dark") in UserDefaults; expose `colorScheme: ColorScheme?` as a computed property returning `nil` for system, `.light`, or `.dark`. The nil value means "follow the system setting" — SwiftUI interprets nil as no preference. Requires `import SwiftUI` at the top of AppSettings.swift.

- **OnboardingView pattern — fullScreenCover + hasSeenOnboarding**: gate the onboarding with `@State private var showOnboarding = !AppSettings.shared.hasSeenOnboarding` in `YomiApp`. Use `.fullScreenCover(isPresented: $showOnboarding)`. On the last page, set `AppSettings.shared.hasSeenOnboarding = true` and call `dismiss()`. The flag persists across launches via UserDefaults — onboarding never appears again after first completion.

- **seedBundledPlugins call removed from YomiApp**: the `seedBundledPlugins()` call in `YomiApp.init()` was removed for App Store compliance (binary must ship zero plugin files). The method itself is kept in `ExtensionManager` for dev/testing use. The `.js` files remain in `Yomi/Resources/` on disk but are no longer seeded — full removal from the Xcode target is a future step before App Store submission.

- **WKWebView re-inject pattern (Coordinator.lastHTML)**: `ReaderWebView` uses a `Coordinator` with a `lastHTML: String` property. In `updateUIView`, compare `html != context.coordinator.lastHTML` before calling `webView.loadHTMLString`. This prevents WebView from reloading on every SwiftUI redraw (e.g. when font slider moves). Only reload when the HTML content actually changes. The styledHTML computed var in `TextReaderView` rebuilds on every change to `fontSize`, `isDarkMode`, `isSepia` — the coordinator check makes this safe.

- **Immersive tap layer pattern in ZStack**: when a reader ZStack contains scrollable/zoomable content, add a `Color.clear.contentShape(Rectangle()).onTapGesture { ... }` layer immediately below the reader content (after the background color). This provides a tap target for the entire screen without blocking scroll or pinch gestures on the layers above it. If the tap layer is placed on top, it intercepts all scroll events.
