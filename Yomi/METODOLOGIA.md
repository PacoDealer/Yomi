...
| 51 | 2026-04-29 | CF bypass reliability + settings UX restructure. (1) `CFBypassView.swift`: `AutoBypassHelper` fixed — WKWebView now full-size off-screen (Cloudflare Turnstile JS checks viewport dimensions; 1×1pt broke the challenge). Timeout extended 10s → 30s. Poll restarts on `didReceiveServerRedirectForProvisionalNavigation` (CF redirects before `didFinish`). UA unified via `CFBypassConstants.userAgent` enum (`nonisolated(unsafe) static let`) — cf_clearance is bound to the UA that solved the challenge; JSBridge `URLSession` now uses the same UA so cookies are accepted on subsequent requests. `BrowseView`: auto-bypass failure shows a dismissible `safeAreaInset` banner with prompt to use manual bypass. (2) `SettingsView.swift`: 12 sections → 7. `MangaReaderSettingsView` + `NovelReaderSettingsView` + `UpdatesSettingsView` + `SuwayomiSettingsView` + `OPDSSettingsView` added as private sub-view structs in same file. `ConnectionTestStatus` promoted to file-level private enum. Downloads section dissolved: auto-scroll + webtoon margins moved to Manga Reader sub-screen; delete-after-reading + concurrent downloads moved inline into Library section. Incognito toggle moved from Reader → General. Sources & Servers: plugin repos inline + NavigationLink to Suwayomi + NavigationLink to OPDS (eliminates 2 separate root sections). Key learning: Swift 6 `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` makes all static properties implicitly @MainActor — use `nonisolated(unsafe) static let` for `Sendable` constants shared across isolation contexts (generates a style warning but is correct and safe). | 45 | 2026-04-23 | Cloudflare auto-bypass + LNReader/Mangayomi plugin fixes. (1) `CFBypassView.swift`: manual full browser sheet. (2) `CFBypassManager` + `AutoBypassHelper`: hidden WKWebView, polls for `cf_clearance`, 10s timeout, `withCheckedContinuation<Bool>`. (3) `JSBridge.swift` CF detection: `ObjectIdentifier(ctx)` keyed dict, CF-RAY header or 403+"Just a moment" detection, `cfBlockedURL`/`clearCFBlock()` instance props. (4) `SourceBrowseView.loadWithBypass()`: auto-bypass flow with `isBypassing` overlay. (5) LNReader v3 `module.exports` fallback in `injectLNReaderAdapter`. (6) Mangayomi `.js`-only filter in `PluginCatalogService`. (7) `injectRequireShim` additions: `@libs/fetch` → `{fetchApi: fn}` wrapping `SOURCE._fetchSync`; `@libs/novelStatus` → `{NovelStatus:{Ongoing,Completed,Unknown}}`; `dayjs` → lightweight stub (subtract/add/format/isValid). (8) `CFBypassView` `initialURL` param — `init` initializes `@State var urlText` from blocked URL so manual bypass sheet opens on the right domain. |
