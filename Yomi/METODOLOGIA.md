...
| 45 | 2026-04-23 | Cloudflare bypass. New `CFBypassView.swift`: full browser sheet (`UIViewRepresentable` WKWebView + editable URL bar). `CFWebViewRepresentable.Coordinator` polls `WKHTTPCookieStore.getAllCookies` every 0.8s; on `cf_clearance` detection copies all domain cookies to `HTTPCookieStorage.shared` — `URLSession.shared` picks them up automatically (no JSBridge changes). Success banner + "Done" enables `onSuccess()` callback. Multiple domains per session supported. `BrowseView.swift` (SourceBrowseView): `shield.slash` toolbar button → sheet → on success calls `loadContent()`. Cleanup: removed dead `YomiCatalogEntryRow` struct from `PluginsView.swift` (replaced by `CatalogGroupRow` in S44, was unreferenced). Key design insight: storing cookies in `HTTPCookieStorage.shared` is the cleanest integration point — zero changes to the JS/Swift fetch pipeline, bypass is fully transparent. |
