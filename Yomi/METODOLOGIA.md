...
| 7 | 2026-03-15 | UX audit (visual + code). UX fixes: NSFW filter default off en PluginsView, BrowseView picker bajo el título con .inline title mode, Settings .navigationTitle. AppSettings singleton (@Observable + UserDefaults). SettingsView (General / Reader manga / Reader novel / Appearance / About). InsightsView (total reading time + per-manga list). DB migration v4 (readingSeconds INTEGER en manga + novel). ChapterReaderView: reading time tracking en .onDisappear, keepScreenOn via UIApplication.isIdleTimerDisabled, defaultReaderMode desde AppSettings. MoreView restructurada: Settings + Plugins + Reading Insights + About. AboutView + LicensesView inline. |
...
- **GRDB + Swift 6 strict concurrency**: exponer DatabaseQueue como un `nonisolated(unsafe) var appDatabase: DatabaseQueue!` a nivel de módulo en lugar de como propiedad de un singleton MainActor-isolated. Este es el patrón oficial GRDB para `SWIFT_DEFAULT_ACTOR_ISOLATION=MainActor`. Todos los métodos de `*Queries` acceden a `appDatabase` directamente — sin actor hop.
- **\*Queries enums**: todos los métodos static deben ser `nonisolated` o el compilador infiere aislamiento MainActor (por `SWIFT_DEFAULT_ACTOR_ISOLATION`) y bloquea las llamadas desde `Task.detached`.
