...
- **External skill/plugin ecosystem survey (2026-08-19)** — 19 GitHub repos + a broader marketplace survey evaluated for anything Yomi-relevant. Verdict: nothing worth installing. `obra/superpowers` (274k★ agentic-skills framework) was cloned and read at the code level — its TDD/verification/debugging skills are just narrated versions of discipline this project already practices with real evidence (live simulator verification, `sqlite3`/`defaults` inspection, explicit "verified vs. not verified" documentation in every session entry above); its `finishing-a-development-branch` skill actively conflicts with this repo's "every session ends with commit+push, no exceptions" rule. `kunchenguid/no-mistakes` (AI push-gate) was also ruled out — it requires pushing to a separately-named git remote rather than hooking real `git push`, which just recreates the "have to remember the extra step" problem it's meant to solve, and its own source runs the Claude CLI with `--dangerously-skip-permissions` inside an unattended background daemon. `saltbo/zpan` was evaluated as a self-hosted CloudKit alternative (Known Issue #47) — its IDs are random, not content-derived, so most of its code solves a conflict-resolution problem this project's content-derived `Manga.id`/`Chapter.id` design (S102) already avoids by construction; not worth building, pay the $99/yr Apple Developer Program instead when ready to publish. See below for the one piece that *was* worth keeping.

### Code review technique — two-stage subagent review (adopted 2026-08-19, from `obra/superpowers`)
The one genuinely reusable idea from the superpowers evaluation above: for a `/code-review` pass with real stakes (pre-submission, or after a large multi-file change), dispatch a **fresh, context-isolated subagent as the implementer/fixer** and a **separate fresh subagent as the reviewer**, rather than reviewing your own just-written code in the same context. The reviewer should be told explicitly to rate each finding Critical/Important/Minor and check spec-compliance, not just style. Loop fixes back through a bounded number of rounds (2-3) rather than open-ended iteration. Fresh eyes catch what the same context tends to rationalize past. Use this as a technique inside existing `/code-review` sessions — not a reason to install any external tool.

### Remote/mobile access (established 2026-08-19)
Yomi's Xcode/simulator dependency means it can only be worked on remotely via Claude Code's **Remote Control** feature (phone/browser drives a session running ON this Mac, not a cloud sandbox) — a cloud-only session has no Xcode and can't build/run/screenshot the app at all. To use: `claude remote-control` (or `claude --remote-control`) on this Mac, then pair from the Claude app or claude.ai/code via the QR code / session URL. **The Mac must stay awake and network-reachable the whole time** — set System Settings → Energy → "Prevent automatic sleeping when the display is off" (or run `caffeinate`) before relying on this while away. Verify the pairing actually works across a different network (not just same-LAN) before trusting it for real remote work.
