...
│   ├── Reader/
│   │   ├── ChapterReaderView.swift  # RTL manga + webtoon, zoom+pan, overlay, prev/next chapter via currentChapterIndex+navigateToChapter, reading timer, MAL tracking. `bridge` is optional (S120) — nil for a Suwayomi manga; all 3 page-fetch sites go through fetchPages(bridge:path:), which routes to the JS plugin (detached, _fetchSync blocks) or SuwayomiService.fetchPageURLs
...
│   ├── More/
│   │   ├── MoreView.swift           # Root More tab (S95 — N.10 spec). Custom card system (mono section headers, s1 cards, 29×29 icon chips) replacing native List/Section. Same grouping as before (App/Library/Sources/Reading/Tracking/Data/About), pure restyle. Includes dedicated AboutView (Version/Build, GitHub/Report-a-bug/Privacy-Policy links, Licenses) — split out of SettingsView's old About section to match N.10/N.07's own split.
│   │   ├── PluginsView.swift        # Installed plugins + Browse catalog (PluginCatalogService, Install button per entry) + NSFW filter
│   │   ├── SettingsView.swift       # General / Reader manga / Reader novel / Appearance (→AppearanceStudioView NavigationLink + "Pure black (OLED)" toggle bound to canvas=="Midnight") / Library (custom pill stepper) / Sources & Servers / Advanced / About (compact Version+GitHub only, S95 — N.07 spec). Glass nav bar (back chip + centered title) matching Detail/Downloads/Insights.
...
│   │   ├── MangaTracker.swift       # Shared protocol every tracker conforms to (S115) — authURL/handleCallback/searchManga/updateProgress/logout/isLoggedIn + pendingAuthState (S120). Carries makeAuthState()/verifyAuthState(url:requireEcho:) (S120), the OAuth CSRF `state` all 4 services now send and verify single-use (Known Issues #123/#124), plus refreshAccessToken()/performTokenRefresh()/sendAuthorized() (S120) — every authenticated tracker call refreshes and retries once on a 401 instead of failing forever (Known Issue #108). Also carries sendProgressUpdate(_:graphQL:) (S119), the one progress-write path all 4 services use: status check + AniList GraphQL errors-in-a-200 check, reported via errorMessage (Known Issue #147)
...
│       ├── SuwayomiService.swift    # REST client for Suwayomi server: fetchSources, fetchPopular, fetchLatest (S88), fetchSearch, fetchMangaDetail, fetchChapters, fetchPageURLs, pageURLs, toManga, toChapter. ID: "suwayomi_{sourceId}_{mangaId}"; chapter path "suwayomi://{mangaId}/{chapterIndex}" (S120). Detail/chapters/pages are consumed by MangaDetailView.loadSuwayomiChapters() + ChapterReaderView.fetchPages — before S120 they had zero call sites and a Suwayomi manga could never show chapters (Known Issue #131)
...
│       └── CFBypassView.swift       # Manual CF bypass sheet + CFBypassManager enum (auto-bypass: full-size off-screen WKWebView — Turnstile needs a real viewport, polls httpCookieStore every 0.5s, copies cf_clearance to HTTPCookieStorage.shared, 30s timeout); opened by shield toolbar button in SourceBrowseView
├── Sync/
...
├── Core/YomiToast.swift             # `.yomiToast(_:)` view modifier (S100) — self-dismissing top banner for surfacing failures that would otherwise be silent (generalizes UpdatesView's ad-hoc refresh-summary banner). `YomiHaptics.error()` alongside it. Used by ChapterReaderView.markChapterRead and MangaDetailView.toggleLibrary's catch blocks.
...
- **Auto-bypass** (`autoBypass(url:) async → Bool`): attaches a full-size off-screen WKWebView to keyWindow for up to 30s — Cloudflare Turnstile needs a real viewport to run its JS, so this is not hidden/1×1pt. Polls `httpCookieStore` every 0.5s for `cf_clearance`. On success: copies all domain cookies to `HTTPCookieStorage.shared` → returns `true`. On timeout → returns `false`. No UI shown.
...
→ Task.detached: decodeBackup(at:) — Data(contentsOf:) → JSONSerialization
  → decodeManga / decodeChapter / decodeNovel / decodeNovelChapter → DecodedBackup (Sendable)
→ ONE appDatabase.write transaction for every row (manga/chapters/categories/novels/links)
...
## On-device Keiyoushi (S124 — shipped in the personal build)

`Yomi/Features/Keiyoushi/`: `KeiyoushiJVMHost.mm` dlopens `Frameworks/OpenJDKRuntime.framework` (OpenJDK Mobile,
Zero, java.base only; device-only, embedded by the 'Embed Keiyoushi runtime' build phase when
`YOMI_EMBED_KEIYOUSHI=YES`) and starts our patched M-Extension-Server (`Keiyoushi/MExtensionServer.jar`) on
127.0.0.1. `KeiyoushiBridge` speaks `POST /dalvik` (APK once per launch, then a handle; 409 → resend; CF-bypass UA +
site cookies forwarded; 403 → Cloudflare prompt). `KeiyoushiRepository` decodes the user-pasted Mihon `index.pb` and
keeps installed APKs + `installed.json` in Application Support/Keiyoushi. IDs: manga `keiyoushi_<mihonId>_<urlhash>`,
sourceId `keiyoushi_<mihonId>`, chapter path `keiyoushi://<base64 json>`; routed like Suwayomi in MangaDetailView and
ChapterReaderView. Not yet routed: Updates, Downloads, backup import.
