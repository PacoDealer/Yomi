...
│   │   ├── MangaTracker.swift       # Shared protocol every tracker conforms to (S115) — authURL/handleCallback/searchManga/updateProgress/logout/isLoggedIn. Also carries sendProgressUpdate(_:graphQL:) (S119), the one progress-write path all 4 services use: status check + AniList GraphQL errors-in-a-200 check, reported via errorMessage (Known Issue #147)
